...
```
- SSH key:  
  `/Users/kanakamanojgarapati/Downloads/kanakamanoj-gkm.pem`
...
### Test Connection
```
ansible -i ansible/inventory.ini all -m ping --private-key ~/Downloads/kanakamanoj-gkm.pem
```

### Run Playbook
```
ansible-playbook -i ansible/inventory.ini ansible/site.yml --private-key ~/Downloads/kanakamanoj-gkm.pem
```
...
Backend:
```
curl http://16.144.31.60:5000/metrics
```
Prometheus:
```
http://16.144.31.60:9090
```
...
Example:
```
ab -n 1000 -c 50 http://16.144.31.60/api/some-endpoint
```
